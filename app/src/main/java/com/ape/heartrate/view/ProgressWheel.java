...
public class ProgressWheel extends View {
    ...
    //Modify by Dave
    private int m_Max = 100;// default is 100
    ...
    private SizeChangedListener m_SizeChangedListener;
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        layout_width = w;
        layout_height = h;
        if (m_SizeChangedListener != null) {
            m_SizeChangedListener.onSizeChanged(this);
        }
        setupBounds();
        setupPaints();
        invalidate();
    ...
    public void setOnSizeChangedListener(SizeChangedListener listener) {
        m_SizeChangedListener = listener;
    }
    ...
    private void setupBounds() {
        ...
        log(
                "layout_width is " + layout_width +
                        " layout_height is " + layout_height +
                        " paddingTop is " + paddingTop +
                        " paddingBottom is " + paddingBottom +
                        " paddingLeft is " + paddingLeft +
                        " paddingRight is " + paddingRight +
                        " width is " + width +
                        " height is " + height
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //Draw the bar
        if (isSpinning) {
            canvas.drawArc(circleBounds, m_Degree - 90, barLength, false,
                    barPaint);
        } else {
     ...
     */
    public void resetCount(boolean showPercent) {
